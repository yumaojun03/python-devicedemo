...
from devicedemoclient import version
from devicedemoclient.common import cliutils
from devicedemoclient import exceptions as exc
...
class DevicedemoClientArgumentParser(argparse.ArgumentParser):
    ...
    def error(self, message):
        ...
        :param message type string, the message information.
        """
...
class DevicedemoShell(object):

    def get_base_parser(self):
        parser = DevicedemoClientArgumentParser(
            prog='devicedemo',
            description=__doc__.strip(),
            epilog='See "devicedemo help COMMAND" '
...
#            help="Set HTTP call timeout (in seconds)")
    ...
    def get_subcommand_parser(self, version):
        ...
        try:
            actions_modules = {
                '1': v1_shell.COMMAND_MODULES
            }[version]
        except KeyError:
            actions_modules = v1_shell.COMMAND_MODULES
...
def main():
    try:
        DevicedemoShell().main(map(encodeutils.safe_decode, sys.argv[1:]))

    except Exception as e:
        logger.debug(e, exc_info=1)
        print("ERROR: %s" % encodeutils.safe_encode(six.text_type(e)), file=sys.stderr)
        sys.exit(1)
